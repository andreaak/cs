...
<body>
    <p>
        ...
        который создается по принципу ASP.NET страницы и может состоять из обычных серверных элементов управления и разметки.
    </p>
    <p>
        Пользовательские элементы управления используются для того, чтобы избавится от дублирования разметки и логики на разных страницах одного веб приложения. Например, необходимо на разных странах выводить новостную ленту, вместо дублирования кода разметки и логики получения новостей из базы на каждой странице, можно перенести эту логику в ascx файл, и использовать его на всех страницах.
    </p>
    <p>
    ...
    <ul>
        <li>Также, зарегистрировать контрол можно перенеся его из окна Solution Explorer в дизайнер страницы.</li>
    </ul>
    <p>
        Каждое открытое свойство контрола может быть использовано как атрибут на странице, которая использует контрол.
        Определив свойство в классе контрола появляется возможность установить значение данного свойства 
        декларативно(в разметке *.aspx файла через атрибут) и программно(в .aspx.cs файле).
    </p>
    <p>
        Каждое событие созданное в контроле может быть использовано в разметке страницы, которая использует контрол.<br />
        ...
        чтобы декларативно создать обработчик на это событие, необходимо задать значение атрибута OnTest.
    </p>
    <p>
    ...
    <ul>
        ...
        <li>Вместо вызова конструктора пользовательского контрола необходимо вызвать метод LoadControl.</li>
